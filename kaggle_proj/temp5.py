...
# read in data
model = XGBClassifier(max_depth=5, n_estimators=200, objective='rank:pairwise')
...
prediction2 = model2.predict(data_t[:, 6:12], verbose=1)
prediction2 = normalize(prediction2)

print ('shapes: ', prediction.shape, prediction2.shape)

myPrediction = np.zeros((len(data_t)))

for i in range(len(data_t)):
    myPrediction[i] = prediction[0][i] + 0.5 * prediction2[i][0]
...
output = open('output/public4.csv', 'w')
