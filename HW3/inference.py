...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions: newBeliefs[p] = 0.0
        
        for p in self.legalPositions:

            if self.beliefs[p] > 0:
                newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
                for newP in newPosDist:
                    newBeliefs[newP] += self.beliefs[p] * newPosDist[newP]

        for p in self.beliefs:
            self.beliefs[p] = newBeliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        beliefs.normalize()
        self.samples = [util.sample(beliefs, self.legalPositions) for i in range(self.numParticles) ]
    ...
    def observe(self, observation, gameState):
        ...
        #util.raiseNotDefined()
        if noisyDistance == None:
            self.samples = [self.getJailPosition()] * self.numParticles
            return
        oldBeliefs = self.getBeliefDistribution()
        newSamples = util.Counter()

        for p in oldBeliefs:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            newSamples[p] += oldBeliefs[p] * emissionModel[trueDistance]

        if newSamples.totalCount() == 0:
            for p in self.legalPositions: newSamples[p] = 1.0

        newSamples.normalize()
        self.samples = [util.sample(newSamples, self.legalPositions) for i in range(self.numParticles) ]
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.legalPositions:
            if oldBeliefs[p] > 0:
                newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
                for newP in newPosDist:
                    newBeliefs[newP] += oldBeliefs[p] * newPosDist[newP]

        newBeliefs.normalize()
        self.samples = [util.sample(newBeliefs, self.legalPositions) for i in range(self.numParticles) ]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        import itertools
        values = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        ...
        for i in range(self.numParticles):
            self.particles.append(self.values[i % len(self.values)])
    ...
    def observeState(self, gameState):
        ...
        newSamples.normalize()
        self.particles = [util.sample(newSamples, self.values) for i in range(self.numParticles) ]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                samplePos = util.sample(newPosDist)
                newParticle[i] = samplePos
